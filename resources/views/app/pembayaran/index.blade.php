                ...
                @foreach ($pembayaran as $item)
                    <tr>
                        ...
                        <td class="flex gap-3 px-4 py-4 text-sm whitespace-nowrap" x-data="{ open: false }">
                            ...
                            @endhasrole
                            @hasrole('Mahasiswa')
                                <x-modal titleDialog="Pembayaran" titleButton="Pembayaran"
                                    msg="Apakah anda yakin ingin memverifikasi pembayaran ini?">
                                    <form method="POST" action="{{ route('app.pembayaran.update', $item->id) }}"
                                        enctype="multipart/form-data">
                                        @method('PUT')
                                        @csrf
                                        <div class="mt-4 ">
                                            <x-file-upload id="pembayaran" label="Bukti Pembayaran"
                                                name="bukti_pembayaran"></x-file-upload>
                                            <x-error-validation name="bukti_pembayaran"></x-error-validation>
                                        </div>
                                        <div class="mt-4">
                                            <x-botton label="Verifikasi" type="submit">
                                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
